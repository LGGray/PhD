...
for (cell in levels(pbmc)){
  ...
  res$FDR <- qvalue(p = res$PValue)$qvalues
  cell = gsub("/|-| ", "_", cell)
  write.table(res, paste0("differential.expression/edgeR/", cell, ".nobatch.txt"),
              row.names=F, sep="\t", quote = F)
}

print("Done with edgeR-QLF no batch")
...
for (cell in levels(pbmc)){
  ...
  result <- FindMarkers(pbmc.cell, slot='counts', ident.1 = "disease", ident.2 = "control",
                             test.use = "MAST", latent.vars=c('SV1', 'SV2'), min.pct = 0, logfc.threshold = 0)                          
...
for (cell in levels(pbmc)){
  ...
  Idents(pbmc.cell) <- "condition"
  result <- FindMarkers(pbmc.cell, slot='counts', ident.1 = "disease", ident.2 = "control",
                             test.use = "MAST", min.pct = 0, logfc.threshold = 0)                          
  result <- cbind(gene = rownames(result), result)
  cell = gsub("/|-| ", "_", cell)
  write.table(result, paste0("differential.expression/MAST/", cell, "nobatch.txt"),
              row.names=F, sep="\t", quote = F)
}
