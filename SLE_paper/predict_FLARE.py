...
X_test['class'] = X_test['class'].map({'control': 0, 'disease': 1})
# Convert ancestry to binary values
X_test['ancestry'] = X_test['ancestry'].replace({"European": 0, "Asian": 1})

# Set class as target variable
y_test = X_test[['class']]

# Check which features are missing and fill with 0
missing = np.setdiff1d(features, X_test.columns)
X_test[missing] = 0
...
metrics.to_csv(f'FLARE/figures/metrics_{cell}_split_{sys.argv[1]}.csv', index=False)
...
# Set the title
ax.set_title(f'Split_{sys.argv[1]}: {cell.replace("_", " ")}')
...
plt.tight_layout()
# Save the figure
plt.savefig(f'FLARE/figures/confusion_{cell}_split_{sys.argv[1]}.pdf', bbox_inches='tight')
plt.close()
...
disp.plot()
disp.ax_.set_title(f'Split_{sys.argv[1]}: {cell.replace("_", " ")}')
plt.savefig(f'FLARE/figures/PRcurve_{cell}_split_{sys.argv[1]}.pdf', bbox_inches='tight')