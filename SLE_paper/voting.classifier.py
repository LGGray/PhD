...
metrics.to_csv(f'pseudobulk/split_{sys.argv[2]}/{sys.argv[3]}/ensemble/metrics_'+os.path.basename(file).replace('.RDS', '')+'.csv', index=False)

# Save confusion matrix to file
confusion = pd.DataFrame(confusion_matrix(y_test, y_pred))
confusion.to_csv(f'pseudobulk/split_{sys.argv[2]}/{sys.argv[3]}/ensemble/confusion_'+os.path.basename(file).replace('.RDS', '')+'.csv', index=False)
...
plt.tight_layout()
# Save the figure
plt.savefig(f'pseudobulk/split_{sys.argv[2]}/{sys.argv[3]}/ensemble/confusion_'+os.path.basename(file).replace('.RDS', '')+'.pdf', bbox_inches='tight')
plt.close()
...
plt.savefig(f'pseudobulk/split_{sys.argv[2]}/{sys.argv[3]}/ensemble/PRcurve_'+os.path.basename(file).replace('.RDS', '')+'.pdf', bbox_inches='tight')

# Save the model
import pickle
filename = f'pseudobulk/split_{sys.argv[2]}/{sys.argv[3]}/ensemble/'+os.path.basename(file).replace('.RDS', '')+'.sav'
pickle.dump(voting_clf, open(filename, 'wb'))

# Save features to file
if sys.argv[3] == 'intersection':
    features.to_csv(f'pseudobulk/split_{sys.argv[2]}/features/intersection_'+os.path.basename(file).replace('.RDS', '')+'.csv', index=False)
elif sys.argv[3] == 'combined':
    features.to_csv(f'pseudobulk/split_{sys.argv[2]}/features/combined_'+os.path.basename(file).replace('.RDS', '')+'.csv', index=False)