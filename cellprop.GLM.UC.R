...
pbmc <- readRDS('datasets/GSE125527/pbmc.female.RDS')

metadata <- pbmc@meta.data

cell_type_counts <- metadata[,c("disease_assignment", "patient_assignment", "predicted.celltype.l2")]

cell_type_counts %>% count(patient_assignment) -> total_counts

left_join(cell_type_counts, total_counts, 
          "patient_assignment") -> cell_type_counts
colnames(cell_type_counts)[3] <- "cluster"
colnames(cell_type_counts)[4] <- "total"

cell_type_counts %>% group_by(disease_assignment, patient_assignment, total) %>% 
...
df_a %>% filter(disease_assignment %in% c('diseased', 'healthy')) -> df
formula = cbind(count, other) ~ cluster * disease_assignment
model1 <- glm(formula = formula, family = 'binomial', data = df)

summary(model1)

emm1 <- emmeans(model1, specs = revpairwise ~ disease_assignment | cluster)
emm1$contrasts %>%
  summary(infer = TRUE, type = 'response') %>%
  rbind() %>%
...
pdf("datasets/GSE125527/cellprop.GLM.pdf")
(
  ...
  + scale_y_log10()
  + theme_minimal()
  + labs(y = 'UC / Control (odds ratio)', title = 'Cell type proportion change', x = 'Average abundance (probability)')
)
dev.off()