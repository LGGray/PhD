...
cell_type_counts <- metadata[,c("RA", "age", "individual", 
...
cell_type_counts %>% group_by(RA, age, individual, total) %>% 
...
formula = cbind(n, other) ~ cluster * RA + cluster 
...
pdf("~/plots/OneK1K/Cell.proportions/all.sc.GLM.pdf")
(
...
write.table(m_results, file="~/datasets/OneK1k/all.sc.proportions.txt", row.names=F, quote=F, sep="\t")
