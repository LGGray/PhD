...
pbmc <- readRDS("~/datasets/SDY998/pbmc.RDS")

metadata <- pbmc@meta.data

cell_type_counts <- metadata[,c("disease", "sample", "predicted.celltype.l2")]
...
cell_type_counts %>% group_by(disease, sample, total) %>% 
...
cell_type_counts <- subset(cell_type_counts, total > 100)
...
df_a %>% filter(disease %in% c('RA', 'OA')) -> df
formula = cbind(count, other) ~ cluster * disease
