...
metrics = pd.DataFrame({'Accuracy': [accuracy], 
                        ...
                        'Kappa': [kappa]})
metrics.to_csv(f'new_pseudobulk/split_{sys.argv[2]}/{sys.argv[3]}/metrics/RF_metrics_'+os.path.basename(file).replace('.RDS', '')+'.csv', index=False)

# Save confusion matrix to file
confusion = pd.DataFrame(confusion_matrix(y_test, y_pred))
confusion.to_csv(f'new_pseudobulk/split_{sys.argv[2]}/{sys.argv[3]}/metrics/RF_confusion_'+os.path.basename(file).replace('.RDS', '')+'.csv', index=False)
...
plt.tight_layout()
# Save the figure
plt.savefig(f'new_pseudobulk/split_{sys.argv[2]}/{sys.argv[3]}/confusion/RF_'+os.path.basename(file).replace('.RDS', '')+'.pdf', bbox_inches='tight')
plt.close()
...
plt.savefig(f'new_pseudobulk/split_{sys.argv[2]}/{sys.argv[3]}/AUROC/RF_'+os.path.basename(file).replace('.RDS', '')+'.pdf', bbox_inches='tight')
...
plt.savefig(f'new_pseudobulk/split_{sys.argv[2]}/{sys.argv[3]}/PRC/RF_'+os.path.basename(file).replace('.RDS', '')+'.pdf', bbox_inches='tight')

# Save the model
import pickle
filename = f'new_pseudobulk/split_{sys.argv[2]}/{sys.argv[3]}/ML.models/RF_model_'+os.path.basename(file).replace('.RDS', '')+'.sav'
