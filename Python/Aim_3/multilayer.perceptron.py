...
grid_search = GridSearchCV(mlp, param_grid, cv=RepeatedKFold(n_splits=10, n_repeats=3, random_state=42), n_jobs=8, verbose=1)
...
confusion.to_csv(f'new_pseudobulk/split_{sys.argv[2]}/{sys.argv[3]}/metrics/MLP_confusion_'+os.path.basename(file).replace('.RDS', '')+'.csv', index=False)
...
plt.tight_layout()
# Save the figure
plt.savefig(f'new_pseudobulk/split_{sys.argv[2]}/{sys.argv[3]}/confusion/MLP_'+os.path.basename(file).replace('.RDS', '')+'.pdf', bbox_inches='tight')
plt.close()
...
plt.savefig(f'new_pseudobulk/split_{sys.argv[2]}/{sys.argv[3]}/AUROC/MLP_'+os.path.basename(file).replace('.RDS', '')+'.pdf', bbox_inches='tight')
...
plt.savefig(f'new_pseudobulk/split_{sys.argv[2]}/{sys.argv[3]}/PRC/MLP_'+os.path.basename(file).replace('.RDS', '')+'.pdf', bbox_inches='tight')

# Save the model
import pickle
filename = f'new_pseudobulk/split_{sys.argv[2]}/{sys.argv[3]}/ML.models/MLP_model_'+os.path.basename(file).replace('.RDS', '')+'.sav'
