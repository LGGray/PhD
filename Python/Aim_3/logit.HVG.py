...
features = logit.feature_names_in_
...
metrics = pd.DataFrame({'Accuracy': [accuracy], 
                        ...
                        'F1': [f1], 
                        'AUC': [auc]})
metrics.to_csv('exp.matrix/metrics/logit_metrics_'+os.path.basename(file).replace('.RDS', test)+'.csv', index=False)

# Save confusion matrix to file
confusion = pd.DataFrame(confusion_matrix(y_test, y_pred))
confusion.to_csv('exp.matrix/metrics/logit_confusion_'+os.path.basename(file).replace('.RDS', test)+'.csv', index=False)
...
plt.savefig('exp.matrix/AUROC/logit_'+os.path.basename(file).replace('.RDS', test)+'.pdf', bbox_inches='tight')
...
disp.plot()
disp.ax_.set_title('logit: ' + os.path.basename(file).replace('.RDS', test).replace('.', ' '))
plt.savefig('exp.matrix/PRC/logit_'+os.path.basename(file).replace('.RDS', test)+'.pdf', bbox_inches='tight')

# Save the model
import pickle
filename = 'ML.models/logit_model_'+os.path.basename(file).replace('.RDS', test)+'.sav'
