...
ensemble_models = []
for i in range(1, 11):
    model = pickle.load(open(f'new_pseudobulk/split_{i}/intersection/ensemble/{cell}.sav', 'rb'))
...
ensemble = VotingClassifier(
    estimators=[
        ...
        ('SVM', SVC(C=average_svm_C, class_weight='balanced', probability=True, random_state=42)),
...
consistent_features = pd.read_csv('new_pseudobulk/figures/top_chrX.consistent.csv')
features = consistent_features[consistent_features['celltype'] == cell.replace('.chrX', '')]['gene'].tolist()

X_train = pd.read_csv(f'new_pseudobulk/split_1/data.splits/X_train.'+cell+'.csv', index_col=0)
y_train = pd.read_csv(f'new_pseudobulk/split_1/data.splits/y_train.'+cell+'.csv', index_col=0)
X_test = pd.read_csv(f'new_pseudobulk/split_1/data.splits/X_test.'+cell+'.csv', index_col=0)
y_test = pd.read_csv(f'new_pseudobulk/split_1/data.splits/y_test.'+cell+'.csv', index_col=0)
