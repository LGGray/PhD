...
def computeGDDAgreement(index1, index2):
    ...
    for i in range(73):
        ...
        for val in allDegrees:
            score1 = values1.get(val, 0)
            score2 = values2.get(val, 0)
...
def computeRGFDistances(graphletCounts, outputName):
    ...
    for i in range(len(graphletCounts) - 1):
        ...
        if pairCount % 1000 == 0:
            print(f'RGF Comparison Jobs submitted: {float(pairCount) / jobCount * 100:.2f}%')
    ...
    while finishedCount < pairCount:
        ...
        if finishedCount % 1000 == 0:
            print(f'RGF Distance comparisons finished: {float(finishedCount) / jobCount * 100:.2f}%')
...
class FinishedConsumer(multiprocessing.Process):
    ...
    def run(self):
        ...
        while finishedCount < self.jobCount:
            ...
            if finishedCount % 1000 == 0:
                print(f'Computation completion: {float(finishedCount) / self.jobCount * 100:.2f}%')
...
def readLeda(networkFile):
    ...
    with open(networkFile, 'r') as fRead:
        mode = 0
        listOfNodes = []
        ...
        while not self.kill_received:
            ...
            try:
                ...
                if self.mode == 4:
                    ...
                    prop2 = numpy.mean(list(dict(network.degree()).values()))
                    netProp = (prop1, prop2)
                elif self.mode == 5:
                    netProp = nx.average_clustering(network)
                elif self.mode == 6:
                    largestComp = max(nx.connected_component_subgraphs(network), key=len)
...
def computeNetworkProperties(allIndexes, mode):
    ...
    for index in allIndexes:
        ...
        if submitCount % 100 == 0:
            print(f'Network Property Jobs Submitted: {float(submitCount) / jobCount * 100:.2f}%')
    ...
    while finishedCount < len(allIndexes):
        ...
        if finishedCount % 100 == 0:
            print(f'Ready Network Properties: {float(finishedCount) / jobCount * 100:.2f}%')
...
def compareDistributions(dist1, dist2):
    ...
    for i in range(1, compLength):
        try:
            norm1 = float(normDist1[i-1]) / N1
        except IndexError:
            norm1 = 0
        
        try:
            norm2 = float(normDist2[i-1]) / N2
        except IndexError:
            norm2 = 0
...
def compareNetworkProps(propertyList, distanceMode, outputNames):
    ...
    for i in range(len(propertyList) - 1):
        ...
        for j in range(i+1, len(propertyList)):
            ...
            if pairCount % 1000 == 0:
                print(f'Jobs submitted: {float(pairCount) / jobCount * 100:.2f}%')
    ...
    while finishedCount < pairCount:
        ...
        if finishedCount % 1000 == 0:
            print(f'Finished comparisons: {float(finishedCount) / jobCount * 100:.2f}%')
    ...
    def run(self):
        while not self.kill_received:
            ...
            try:
                ...
                network = readLeda(f'{ndumpName}.gw')
...
def getSpectralSignatures(allIndexes):
    ...
    for index in allIndexes:
        ...
        if submitCount % 100 == 0:
            print(f'Spectrum Computation Jobs Submitted: {float(submitCount) / jobCount * 100:.2f}%')
    ...
    while finishedCount < len(allIndexes):
        ...
        if finishedCount % 100 == 0:
            print(f'Ready Spectrums: {float(finishedCount) / jobCount * 100:.2f}%')
...
def computeSpectralDists(propertyList, outputName):
    ...
    # Put the jobs to be consumed
    jobCount = len(propertyList) * (len(propertyList) - 1) // 2
    spectList = list(propertyList.keys())
...
def checkInput(ndumpFolder, distType, num_processes):
    if not os.path.isdir(ndumpFolder):
        print(f'ERROR: Provided network folder path is not found: {ndumpFolder}')
        sys.exit(1)
    ...
    if distType == 'rgf':
        ...
        print(f'ERROR: Unknown network distance type: {distType}')
    ...
    for root, _, files in directory:
        for fileName in files:
            if fileName.endswith('.ndump2'):
                ...
                indexName = os.path.join(root, stripName)
                
                # Read networks for tests 4, 5, 6
                if testMode in [4, 5, 6, 17] and stripName + '.gw' not in files:
